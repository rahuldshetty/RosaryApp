...
public class SubMainActivity extends AppCompatActivity {
    ...
    private String evening_text[] = {"ªÀÄíeÁå zÉªÁ (Ordinary)","ªÀÄíeÁå zÉªÁ (Easter)"};
    private String morning_text[] = {"¸ÀPÁ½Aa ¨sÉmÉÆ«Ú (Ordinary)","¸ÀPÁ½Aa ¨sÉmÉÆ«Ú (Easter)"};
        ...
        try
        {
            ...
            for(String file: am.list(path+subPath)){
                ...
                if(file.contains(".pdf")) {
                    ...
                    String title = file.replace(".pdf", "").replace(".","").replaceAll("[0-9]", "");
                    ...
                    String songPath = path + file.replace(".pdf",".mp3");
                    sub.setSongPath(songPath);

                    String imagePath = path + "images/" + file.replace(".pdf",".jpg");
                    InputStream ims = am.open(imagePath);
                    Bitmap bitmap = BitmapFactory.decodeStream(ims);
                    sub.setImage(bitmap);

                    String descPath = path+ subPath + "/desc/" + file.replace(".pdf",".txt");
